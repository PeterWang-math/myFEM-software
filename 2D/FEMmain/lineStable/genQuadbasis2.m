function newbasis = genQuadbasis2(mesh,basis,femtype)
% generate trial basis function space infomation for quadrilateral element
% and FEM nodes infomation in 2d, trial space is considered seperately for PGM
%
% The following input infomation is needed:
% mesh: a struct consisting of mesh.P, mesh.T, mesh.N;
% basis: a struct consisting of basis.trial.type, basis.trial.order
%
% The output information includes:
% basis.trial.reffun: 7 x Nlb x (num_of_derivatives+1) cell consisting of
...
% basis.trial.Pb/Tb/Nb/Nlb: FEM nodes information
...
% the basis function is defined on the reference quadrilateral element (0,1)^2
if strcmp(femtype.type,'pgm')==1 || strcmp(femtype.type,'ipdg')==1 % PG/IPDG method

    if contains(['l','h','cr','a','b','hct','rt'],basis.trial.type) == 0
        warning(['The basis.trial.type must be either l, h, cr, a, b, hct, or rt ',...
            'already reset to l']);
        basis.trial.type = 'l';
    end
    switch basis.trial.type
        case 'l'
            ord = basis.trial.order;
            if floor(ord)~=ord || ord<1 || ord>7
                warning(['The basis.trial.order must be a positive integer less than 8,',...
                    'already reset to 1']);
                basis.trial.order = 1;
            end
            switch basis.trial.order
                case 1 % Lagrange bilinear basis function
                    switch femtype.type
                        case 'pgm'
                    ...
                    %% vectorization programming: every constant C is modified as C+x-x 
                    fclass = cell(7,4,9);
                    %                  fclass{1,1,1} = @(x,y) l1(x,y);
                    %                  fclass{1,2,1} = @(x,y) l2(x,y);
                    %                  fclass{1,3,1} = @(x,y) l3(x,y);
                    %                  fclass{1,4,1} = @(x,y) l3(x,y);
                    fclass{1,1,1} = @(x,y) 1-x-y+x.*y;
                    fclass{1,4,1} = @(x,y) x-x.*y;
                    fclass{1,3,1} = @(x,y) x.*y;
                    fclass{1,2,1} = @(x,y) y-x.*y;

                    fclass{2,1,1} = @(x,y) -1+y;
                    fclass{2,4,1} = @(x,y) 1-y;
                    fclass{2,3,1} = @(x,y) y;
                    fclass{2,2,1} = @(x,y)-y;
                    fclass{2,1,2} = @(x,y) -1+x;
                    fclass{2,4,2} = @(x,y) -x;
                    fclass{2,3,2} = @(x,y) x;
                    fclass{2,2,2} = @(x,y) 1-x;

                    fclass{3,1,1} = @(x,y) 0+x-x;
                    fclass{3,2,1} = @(x,y) 0+x-x;
                    fclass{3,3,1} = @(x,y) 0+x-x;
                    fclass{3,4,1} = @(x,y) 0+x-x;
                    fclass{3,1,2} = @(x,y) 1+x-x;
                    fclass{3,2,2} = @(x,y) -1+x-x;
                    fclass{3,3,2} = @(x,y) 1+x-x;
                    fclass{3,4,2} = @(x,y) -1+x-x;
                    fclass{3,1,3} = @(x,y) 0+x-x;
                    fclass{3,2,3} = @(x,y) 0+x-x;
                    fclass{3,3,3} = @(x,y) 0+x-x;
                    fclass{3,4,3} = @(x,y) 0+x-x;
                    % fclass{1,1:4,2:9} = 0;
                    % fclass{2,1:4,3:9} = 0;
                    % fclass{3,1:4,4:9} = 0;
                    % fclass{4:7,1:4,1:9} = 0;
                    for k=2:9
                        fclass{1,1,k} = @(x,y) 0+x-x;
                        fclass{1,2,k} = @(x,y) 0+x-x;
                        fclass{1,3,k} = @(x,y) 0+x-x;
                        if k>=3
                            for ind=1:4
                                fclass{2,ind,k} = @(x,y) 0+x-x;
                            end
                        end
                        if k>=4
                            for ind=1:4
                                fclass{3,ind,k} = @(x,y) 0+x-x;
                            end
                        end
                    end
                    for i=4:7
