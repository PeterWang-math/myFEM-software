function y = quadError2(mesh,basis,uclass,uh,type,Gauss_order,femtype)
% get error between exact solution u and FEM solution uh in quadrilateral mesh
...
% parallel computing
parfor n=1:mesh_N
   J = [abs(mesh_P(1,mesh_T(4,n))-mesh_P(1,mesh_T(1,n))),0; 
        0,abs(mesh_P(2,mesh_T(2,n))-mesh_P(2,mesh_T(1,n)))];
   w = mesh_P(:,mesh_T(1,n));
   detJ = det(J);
   xhat = intpt*J'+w';
   f = cell(trnlb,1); 
   ...
   y = y + detJ * weight * ipt;  % area(ref_quad)=1;
end
y = abs(sqrt(y));
end
